...
app.post('/api/menus/:id/image', upload.single('file'), async(req , res) => {
	try {
		console.log("파일패스"+ req.file.path)
...
//GET api/orders/:id  주문 내역 상세 조회
app.get("/api/orders/:id", async(req, res) =>{
    try {
        const data = await pool.query("SELECT * FROM orders WHERE id = ?",
         [req.params.id])
        ...
        return res.json({
            success:false,
            message : "주문 내역 상세 조회에 실패하였습니다."
        });        
    }
})

// PATCH /api/orders/:id : 주문 내역 수정
